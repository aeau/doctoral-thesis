...
\textsc{rq iii} & \textsc{iv, viii, ix}\\ %\hline
\textsc{rq iii.i} & \textsc{iv, viii ix}\\ %\hline
...
%In the case of~\textsc{paper iv}, the similarities presented between agents and humans helped us identify characteristics that could be valuable to model for creating adapted content for the end-user.
...
% This means that their design steps influence what areas are explored, and due to the quality-diversity properties of IC MAP-Elites, we can still explore space out of their design, which means that their process  We used their design steps implicitly in the search algorithm By using the designer's design, we could 


%   At the same time, we evaluated th
